...
Function Update-ParamHelp {
  ...
  param ()
    ...
    $localRevision = $LocalMeta.Meta.Revision - $LocalMeta.Meta.Local.Revision

    # Checking for local changes
    if ($LocalMeta.Meta.Local.Modified) {
    ...
    $DownloadedUpdate = DownloadUpdate -Uri $LocalMeta.Meta.Source -ErrorAction Stop
...
function ValidateOneMatch {
    ...
    if (!$jsonobjects){
        Throw "Could not find parameter help content with that name."
...
function GetJsonData {
    ...
    if ($Meta) {
        if (($allobjects | Get-Member -MemberType NoteProperty).Name -notcontains 'Meta') {
            Throw "Json file is not valid. Metadata is missing."
        }
    } 
    $jsonobjects = @()
    foreach ($object in $allobjects.ParamHelp) {
        ...
        #if ($object.Examples) {
            Add-Member -InputObject $object -MemberType ScriptProperty -Name ExamplesCount -Value {
                if (($this | Get-Member -MemberType NoteProperty).Name.Contains('Examples')) {
        ...
        #} else {
            #Add-Member -InputObject $object -MemberType ScriptProperty -Name ExamplesCount -Value 0
        #}
        $jsonobjects += $object
    }
    Update-TypeData -TypeName ParameterHelp -DefaultDisplayPropertySet Name, Type, ExamplesCount -Force -WhatIf:$false
    if ($Meta) {
        $completeObject = New-Object -TypeName psobject -Property @{
            Meta = $allobjects.Meta
            ParamHelp = $jsonobjects
        }
...
function SaveJsonData {
  ...
  if (-not (Test-Path -Path (Split-Path -Path $Path -Parent))) {
    ...
    $originalJson = getJsonData -Path $Path -Meta 
    $originalJson.ParamHelp = $data
    
    # update local meta
    if (!$KeepRevision) {
      $originalJson.Meta.Revision += 1
      $originalJson.Meta.Local.Revision += 1
      $originalJson.Meta.Local.Modified = $true
    }
    
    try {
        $originalJson.ParamHelp = $originalJson.ParamHelp | Sort-Object -Property Name | Select-Object -Property * -ExcludeProperty ExamplesCount
        ConvertTo-Json -InputObject $originalJson -Depth 4 -ErrorAction Stop | Out-File -FilePath $Path -ErrorAction Stop
    } catch {
        Write-Error "Unable to save json file."
    }
}
#endregion